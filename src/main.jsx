...
function Header() {
  ...
  return <header className="header"><Logo /><button className="menu-toggle" aria-label="Toggle navigation" aria-expanded={open} onClick={() => setOpen(!open)}>{open ? 'Close' : 'Menu'} <span>{open ? '−' : '+'}</span></button><nav className={open ? 'open' : ''} aria-label="Main navigation"><ProjectsDropdown onNavigate={() => setOpen(false)} /><a className="nav-cta" href="#products" onClick={() => setOpen(false)}>Explore products <Arrow diagonal /></a><button className="nav-account" type="button" disabled>Login</button><button className="nav-account nav-register" type="button" disabled>Register</button></nav></header>;
}
function Hero() {
